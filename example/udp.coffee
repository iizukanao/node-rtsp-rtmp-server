...
sendAudioStart = (callback) ->
  ...
  try
    audioControlClient.writeReliable buf, callback
  catch e
...
h264.on 'dts_nal_units', (pts, dts, nalUnits) ->
  ...
  try
    if isImportant
      videoDataClient.writeReliable buf
    else
      videoDataClient.write buf
  catch e
